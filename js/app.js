  ...
  */
  ...
  function chainSimplePromise(value) {
      ...
      simpleWrapper(value).then(valueLogger);
  }
  ...
  function writeArticleToDOM(articleText) {
      ...
      // and, just as good form, return a promise to the end of the chain, containing articleText, so we can see the output:
        return simpleWrapper(articleText);
  }
  ...
  function doMoreStuff() {
      ...
      // (Wat? JQuery promises are weird... What do we do?)
       compoundXHR = Q(xhrPromise).then(stripOutMainContent).then(writeArticleToDOM);
    ...
    // NETWORK OPERATION ENDS
      ...
      // IMPLEMENT
      simplePromise = simpleWrapper(simpleValue);
      simplePromise.then(valueLogger);
    ...
    // TIMEOUT PROMISE ENDS
      ...
      Q.all([simplePromise,compoundXHR,delayPromise]).then(valueLogger).then(writeToDOMAgain);
  }
  ...
  // IMPLEMENT
  // docPromise =
  var docDefer = Q.defer();
  var docPromise = docDefer.promise;
  ...
  // IMPLEMENT
  docPromise.then(valueLogger).then(writeToDOM).then(doMoreStuff);